...
const TableData = (props) => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    const {data} = await axios.put(`http://localhost:8080/api/v1/editTransaction/${currId}`, {
      ...values,
    });
  ...
  const handleDeleteClick = async (itemKey) => {
    ...
    setCurrId(itemKey);
    const {data} = await axios.post(`http://localhost:8080/api/v1/deleteTransaction/${itemKey}`,{
